...
func (r *Label) Create(ctx context.Context, req *proto.CreateRequest, rsp *proto.CreateResponse) error {
	if req.Label == nil {
		return errors.BadRequest("go.micro.srv.router.Label.Create", "invalid label")
	}

	if len(req.Label.Id) == 0 {
		return errors.BadRequest("go.micro.srv.router.Label.Create", "invalid id")
	}

	if len(req.Label.Service) == 0 {
		return errors.BadRequest("go.micro.srv.router.Label.Create", "invalid service")
	}

	if len(req.Label.Key) == 0 {
		return errors.BadRequest("go.micro.srv.router.Label.Create", "invalid key")
	}

	if req.Label.Weight < 0 || req.Label.Weight > 100 {
		return errors.BadRequest("go.micro.srv.router.Label.Create", "invalid weight, must be 0 to 100")
	}

	if err := label.Create(req.Label); err != nil {
		return errors.InternalServerError("go.micro.srv.router.Label.Create", err.Error())
	}
